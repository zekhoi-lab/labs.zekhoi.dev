import { Metadata } from 'next'
import HashGenerator from './client'

export const metadata: Metadata = {
  title: 'Hash Generator | SHA-1, SHA-256, SHA-512',
  description: 'Generate secure hashes efficiently. Supports SHA-1, SHA-256, SHA-384, and SHA-512. Fast, client-side, and privacy-focused.',
  keywords: ['hash generator', 'md5 hash', 'sha256 hash', 'sha512 generator', 'online hasher', 'cryptography tools'],
  openGraph: {
    title: 'Hash Generator | SHA-1, SHA-256, SHA-512',
    description: 'Compute cryptographic hashes instantly in your browser. Supports SHA-1, SHA-256, SHA-384, and SHA-512. 100% private.',
    url: 'https://labs.zekhoi.dev/hash',
    siteName: 'Labs by zekhoi',
    locale: 'en_US',
    type: 'website',
    images: '/opengraph-image',
  },
  alternates: {
    canonical: 'https://labs.zekhoi.dev/hash',
  }
}

export default function Page() {
  return <HashGenerator />
}
